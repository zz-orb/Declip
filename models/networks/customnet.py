...
'''
import models.networks.xception
import sys
...
def make_patch_resnet(depth, layername, extra_output, num_classes=2):
...
def make_patch_xceptionnet(layername, extra_output, frontend=None, num_classes=2):
    def change_out(layers):
        ind, layer = [(i, l) for i, (n, l) in enumerate(layers)
                      if n == layername][0]
        ...
        # FC layer for localisation
        # layers[ind+1:] = [('convout', nn.Conv2d(num_ch,
        #                    num_classes, kernel_size=1))]
        layers[ind+1:] = []
        return layers
    model = CustomXceptionNet(
        modify_sequence=change_out, frontend=frontend, extra_output=extra_output)
    return model
...
def make_xceptionnet_long():
    ...
    def change_out(layers):
        ...
        new_layers = [
            # made all strides = 1
            ('pblock3', models.networks.xception.PixelBlock(channels[4], channels[5],
                                                     2, 1, start_with_relu=True, grow_first=True)),
            ('pblock4', models.networks.xception.PixelBlock(channels[5], channels[6],
...
class CustomXceptionNet(nn.Module):
    ...
    def __init__(self, channels=None, num_classes=1000,
        ...
        sequence = modify_sequence(layers + [
            ...
            ('block1', models.networks.xception.Block(channels[2],
                                               channels[3], 2, 2, start_with_relu=False, grow_first=True)),
            ('block2', models.networks.xception.Block(channels[3], channels[4],
                                               2, 2, start_with_relu=True, grow_first=True)),
            ('block3', models.networks.xception.Block(channels[4], channels[5],
                                               2, 2, start_with_relu=True, grow_first=True)),
            ('block4', models.networks.xception.Block(channels[5], channels[6],
                                               3, 1, start_with_relu=True, grow_first=True)),
            ('block5', models.networks.xception.Block(channels[6], channels[7],
                                               3, 1, start_with_relu=True, grow_first=True)),
            ('block6', models.networks.xception.Block(channels[7], channels[8],
                                               3, 1, start_with_relu=True, grow_first=True)),
            ('block7', models.networks.xception.Block(channels[8], channels[9],
                                               3, 1, start_with_relu=True, grow_first=True)),
            ('block8', models.networks.xception.Block(channels[9], channels[10],
                                               3, 1, start_with_relu=True, grow_first=True)),
            ('block9', models.networks.xception.Block(channels[10], channels[11],
                                               3, 1, start_with_relu=True, grow_first=True)),
            ('block10', models.networks.xception.Block(channels[11], channels[12],
                                                3, 1, start_with_relu=True, grow_first=True)),
            ('block11', models.networks.xception.Block(channels[12], channels[13],
                                                3, 1, start_with_relu=True, grow_first=True)),
            ('block12', models.networks.xception.Block(channels[13], channels[14],
                                                2, 2, start_with_relu=True, grow_first=False)),
            ('conv3', models.networks.xception.SeparableConv2d(channels[14], channels[15],
                                                        3, 1, 1)),
            ('bn3', nn.BatchNorm2d(channels[15])),
            ('relu3', nn.ReLU(inplace=True)),
            ('conv4', models.networks.xception.SeparableConv2d(channels[15], channels[16],
                                                        3, 1, 1)),
...
if __name__ == '__main__':
    ...
    model.load_state_dict(model_zoo.load_url(resnet.ResNet18_Weights.IMAGENET1K_V2))
    print('Loading resnet34')
    model = CustomResNet(34)
    model.load_state_dict(model_zoo.load_url(resnet.ResNet34_Weights.IMAGENET1K_V2))
    print('Loading resnet50')
    model = CustomResNet(50)
    model.load_state_dict(model_zoo.load_url(resnet.ResNet50_Weights.IMAGENET1K_V2))
    print('Loading resnet101')
    model = CustomResNet(101)
    model.load_state_dict(model_zoo.load_url(resnet.ResNet101_Weights.IMAGENET1K_V2))
    print('Loading resnet152')
    model = CustomResNet(152)
    model.load_state_dict(model_zoo.load_url(resnet.ResNet152_Weights.IMAGENET1K_V2))
