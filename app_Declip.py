...
def init_detector():
    ...
    global detector
    try:
        detector = DeclipDetector(ckpt='/home/zhaoxiaohan_bishe/Declip/checkpoints/DeCLIP_SOTA/model_epoch_best.pth')
        print("✅ 伪造检测器初始化成功")
...
@app.route('/health_declip', methods=['GET'])
def health_check():
    """健康检查端点"""
...
@app.route('/detect_declip', methods=['POST'])
def detect_forgery():
    """
    伪造检测API接口
    接收图片并进行伪造检测与定位
